...
int main(int argc, char *argv[])
{
    ...
    abcClass abcInstance;
    Info << "Default value " << abcInstance.get() << endl;

    // Set a new value
    abcInstance.set(10);
    Info << "New value " << abcInstance.get() << endl;

    // Call a basic function
    abcInstance.basicFunction();

    // Pass a reference to the mesh to the custom class and let it do its things
    abcInstance.meshOpFunction(mesh);
    Info << "Yet another value " << abcInstance.get() << endl;
